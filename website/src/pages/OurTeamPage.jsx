...
const SocialIcons = ({ member }) => (
  <div className="flex justify-center gap-4 mt-4 text-white">
    {member.github && (
      <motion.a href={member.github} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.3 }}>
        <FaGithub size={20} />
      </motion.a>
    )}
    {member.linkedin && (
      <motion.a href={member.linkedin} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.3 }}>
        <FaLinkedin size={20} />
      </motion.a>
    )}
    {member.instagram && (
      <motion.a href={member.instagram} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.3 }}>
        <FaInstagram size={20} />
      </motion.a>
    )}
  </div>
  ...
  >
    ...
    >
      <div className="relative w-24 h-24 mx-auto mb-4">
        ...
        <img src={member.image} alt={member.name} className="relative z-10 w-full h-full rounded-full border-2 border-white shadow-md" />
      </div>
  ...
  >
    ...
    >
      <div className="relative w-40 h-40 mb-4">
        ...
        <img src={member.image} alt={member.name} className="relative z-10 w-full h-full rounded-full border-4 border-white shadow-lg" />
      </div>
    ...
    >
      ...
      >
        ...
        <p className="mt-4 text-gray-400 text-lg max-w-xl mx-auto font-serif">
          Building the future of assistive technology with creativity, code, and collaboration.
        </p>
      </motion.div>

      <div className="flex justify-center mb-24">
        {teams.leader.map((member, i) => (
          <LeaderCard key={i} member={member} />
        ))}
      </div>

      <TeamSection title="Software Developers" icon={<FaLaptopCode />} members={teams.software} delayOffset={0.3} />
      <TeamSection title="Hardware Engineers" icon={<FaTools />} members={teams.hardware} delayOffset={0.6} />
    </motion.div>
  );
};
